...
function! s:IDE.new(bufnr, ...) abort
  ...
  let new.highlight = g:coqpit#options#get('highlight')
  let new.style_checked = g:coqpit#options#get('highlight_style_checked')
  let new.style_queued = g:coqpit#options#get('highlight_style_queued')

  let new.GoalBuffers = []
  let new.InfoBuffers = []

  let new.coqtop_handler = coqpit#CoqTopHandler#new(l:args)
...
function! s:IDE._goal(state_id, is_err, msg, err_loc) abort
  ...
  if a:is_err
    ...
    if a:msg isnot v:null && !empty(a:msg)
      " a:msg is xml
      let self.goal_message = coqpit#goals#xml2strs(a:msg)
    endif
...
function! s:IDE._after_bufenter() abort
  ...
  if self.focusing()
    if self.unfocused
      ...
      if g:coqpit#options#get('refresh_after_focus')
        call self.refreshGoal()
        call self.refreshInfo()
      endif

      if g:coqpit#options#get('rerun_after_focus')
        call self.rerun()
      endif
    endif
...
function! s:IDE._after_textchange() abort
  ...
  if g:coqpit#options#get('keep_after_textchange')
...
function! s:IDE._check_queue() abort
  ...
  if len(self.queue) == 0 || g:coqpit#options#get('show_goal_always')
  ...
  if g:coqpit#options#get('update_status_always')
...
function! s:IDE.coq_next() abort
  ...
  if !coqpit#annotate#is_ending(content, last)
    return
  endif
  ...
  if g:coqpit#options#get('auto_move')
...
function! s:IDE.coq_expand_to_pos(pos, ...) abort
  ...
  while s:pos_lt(last, a:pos)
    ...
    if last is v:null
      ...
      if coqpit#annotate#is_ending(content, self.queue[-1][1])
...
function! s:IDE.coq_to_cursor(...) abort
  ...
  if l:ceil is v:null
    let l:ceil = g:coqpit#options#get('cursor_ceiling')
  endif
